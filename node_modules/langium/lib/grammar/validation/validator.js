...
export class LangiumGrammarValidator {
    ...
    checkFragmentKeywords(rule, accept) {
        if (rule.fragment) {
            if (rule.dataType || rule.returnType || rule.inferredType) {
                accept('error', 'Fragment rules cannot specify a return type.', { node: rule, property: rule.dataType ? 'dataType' : rule.returnType ? 'returnType' : 'inferredType' });
            }
