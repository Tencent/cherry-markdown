...
import { createNodeAction, readEditableSource, selectEditableSource } from './node-view-utils.js';
...
class MathNodeView implements NodeView {
  ...
  constructor(
    ...
    if (!node.isInline) {
      ...
      controls.className = 'cherry-embed__controls cherry-math__controls';
      this.sourceToggle = createNodeAction('源码', '编辑块公式源码', this.toggleSource, readonly);
      this.sourceToggle.setAttribute('aria-expanded', 'false');
      controls.append(this.sourceToggle);
      this.sourcePanel = document.createElement('pre');
      this.sourcePanel.className = 'cherry-embed__source cherry-math__source';
      ...
      this.source.textContent = String(node.attrs.value ?? '');
      ...
      this.dom.append(controls, this.sourcePanel);
    }
  ...
  update(node: ProseNode) {
    ...
    if (this.source && document.activeElement !== this.source) this.source.textContent = String(node.attrs.value ?? '');
    return true;
  }
  ...
  private commitValue(value: string) {
    const pos = this.getPos();
    if (typeof pos !== 'number' || value === this.node.attrs.value) return;
    this.view.dispatch(this.view.state.tr.setNodeMarkup(pos, undefined, { value, source: '' }));
  }
  ...
  private onSourceInput = () => {
    if (!this.source) return;
    this.commitValue(readEditableSource(this.source));
  };

  private toggleSource = () => {
    if (!this.sourcePanel || !this.sourceToggle || !this.source) return;
    const open = this.sourcePanel.hidden === true;
    ...
    if (open) this.source.focus({ preventScroll: true });
