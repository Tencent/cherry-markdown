...
function leafSchema(
  ...
  return $nodeSchema(name, () => ({
    atom: true,
    defining: !inline,
    // Rendered Cherry blocks contain selectable preview text and controls.
    // Making the atom itself draggable lets the browser turn a text-selection
    // gesture inside Tabs/Columns/Timeline into a drag of the entire node.
    // Block movement must be opt-in through an explicit handle instead.
    draggable: false,
...
function compoundItemContentClass(role: string) {
  if (role === 'detail-item') return ' cherry-detail-body';
  if (role === 'tab') return ' cherry-tabs-item__content';
  if (role === 'timeline-item') return ' cherry-timeline--desc';
  return '';
}
...
class CompoundItemView implements NodeView {
  ...
  constructor(node: ProseNode, view: EditorView, getPos: () => number | undefined, readonly: boolean) {
    ...
    header.addEventListener('click', this.selectItemFromHeader);
    this.contentDOM = document.createElement('div');
    this.contentDOM.className = `cherry-compound-item__content${compoundItemContentClass(role)}`;
  ...
  destroy() {
    this.header.removeEventListener('click', this.selectItemFromHeader);
  }

  private readonly selectItemFromHeader = (event: MouseEvent) => {
    // Wait for a completed click before selecting the whole item. Selecting on
    // mousedown steals drag-selection gestures that begin in the header.
...
function compoundNativeClass(node: ProseNode, kind: string, isPanel: boolean) {
  if (node.type.name === 'cherry_detail') return 'cherry-detail';
  if (isPanel) return `cherry-panel cherry-panel__${kind === 'panel' ? 'primary' : kind}`;
  if (kind === 'tabs') return 'cherry-tabs';
  if (kind === 'timeline') return 'cherry-timeline cherry-timeline__vertical';
  return '';
}
...
class CompoundView implements NodeView {
  ...
  private sync(node: ProseNode) {
    ...
    const cherryClass = compoundNativeClass(node, kind, isPanel);
...
class EmbedView implements NodeView {
  ...
  constructor(
    ...
    this.controls = document.createElement(node.isInline ? 'span' : 'figcaption');
    this.controls.className = 'cherry-embed__controls';
    this.controls.hidden = node.type.name === 'cherry_emoji';
    const nativeBlock = node.type.name === 'cherry_native_block';
    const edit = createNodeAction(
      nativeBlock ? '编辑区块' : '源码',
      nativeBlock ? '编辑整个区块源码' : '在节点内编辑源码',
      this.toggleSource,
      config.readonly,
    );
    this.sourceToggle = edit;
    edit.setAttribute('aria-expanded', 'false');
    this.controls.append(edit);
    ...
    this.dom.append(this.preview, this.controls, this.sourcePanel);
    this.scheduleRender();
    this.syncSource();
  }
  ...
  stopEvent(event: Event) {
    return (
      // Cherry-native compound blocks own their rendered DOM. Keep pointer
      // selection and tab/panel controls in that layer instead of letting
      // ProseMirror turn the first drag into a NodeSelection.
      (this.node.type.name === 'cherry_native_block' && this.preview.contains(event.target as Node)) ||
