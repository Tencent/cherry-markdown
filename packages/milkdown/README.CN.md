...
Cherry 的 edit&preview 与 previewOnly 即见即所得插件。Milkdown/ProseMirror 管理预览区的
文档、选区和历史；当前 Cherry 实例继续提供解析、主题和原生预览控件。
插件不创建第二个 Cherry，也不替换顶部工具栏或增加源码联想面板。
...
`Cherry.usePlugin()` 只调用一次，配置会应用到随后创建的 edit&preview 和 previewOnly
Cherry 实例。每个 Cherry 仍拥有独立的 Milkdown 文档和生命周期；
`cherry.destroy()` 会自动清理插件。editOnly 和 CherryStream 不启用。
如果同一页面的实例需要不同设置，可用 `configure({ cherry, instanceId, mode })`
返回实例级覆盖，无需重复静态注册。

## 配置

- `readonly`：关闭编辑及编辑控件。
- `bubble`：是否复用 Cherry 文本选区格式菜单，默认 true。previewOnly 不显示顶部 Toolbar；双栏保留 Cherry 原顶部 Toolbar。
...
- `configure(context)`：按 Cherry 实例返回局部覆盖，适用于多评论、只读与可编辑实例混用的页面。

普通文本直接编辑；代码块直接编辑并支持语言选择；复杂节点在内部编辑源码。
普通链接保持 Cherry 原有点击行为；悬停或聚焦链接后，可通过链接末尾的浮动按钮查看和修改显示文本与真实地址，后方有正文时按钮会自动避让；也可用 `Command/Ctrl + K` 主动打开。
Columns 等原生块中的表格图表提供独立“源码”入口，只选中实际渲染的表格，不会误选旁边的 fenced 示例。
双栏下 Bubble 根据显式焦点归属工作：左侧选区保持 CodeMirror 原行为，右侧选区作用于 Milkdown 并立即同步源码。Bubble 不对代码块、公式、HTML/图表源码或跨原子节点的选区生效。
选中图片或 Mermaid 可编辑宽度及对齐；进入 Mermaid 源码编辑时，这些布局控件会暂时退出。
...
只有一个 React 页面，默认使用 Cherry previewOnly；通过 `?mode=edit%26preview`
或 `?mode=editOnly` 切换标准 Cherry 模式；
