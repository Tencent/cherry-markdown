...
for (const item of editableCases) {
  test(`focus, select, edit and delete text without shifting ${item.name}`, async ({ page }) => {
    ...
    // Text insertion may naturally change an inline block's intrinsic width;
    // it must not move the block or overflow the page.
...
test('Panel and Tabs text selection never drags or duplicates the containing block', async ({ page }) => {
  const selectTextWithMouse = async (selector: string) => {
  ...
  const panel = page.locator('.cherry-compound.cherry-panel');
  const panelBefore = await panel.boundingBox();
  expect(await selectTextWithMouse('.cherry-panel--body p')).toContain('Panel body selection');
  await expect(panel).not.toHaveClass(/is-selected|ProseMirror-selectednode/);
  expect(await panel.boundingBox()).toEqual(panelBefore);
  expect(await markdown(page)).toBe(':::warning Notice\nPanel body selection\n:::');

  const tabsMarkdown = ':::tabs\n:: One\nFirst tab selection\n:: Two\nSecond tab\n:::';
  await setMarkdown(page, tabsMarkdown);
  const nativeTabs = page.locator('.cherry-embed--cherry_native_block');
  const tabs = nativeTabs.locator('.cherry-tabs');
  const tabsBefore = await tabs.boundingBox();
  await expect(nativeTabs).not.toHaveAttribute('draggable', 'true');
  expect(await selectTextWithMouse('.cherry-tabs p')).toContain('First tab selection');
  expect(await tabs.boundingBox()).toEqual(tabsBefore);
  await tabs.getByText('Two', { exact: true }).click();
  await expect(tabs.getByText('Second tab', { exact: true })).toBeVisible();
  expect(await markdown(page)).toBe(tabsMarkdown);
});
...
test('inline and block formula edits update Markdown without opening text Bubble', async ({ page }) => {
  ...
  const math = page.locator('.cherry-math--block');
  const sourceToggle = math.getByRole('button', { name: '编辑块公式源码' });
  await sourceToggle.click();
  await expect(sourceToggle).toHaveAttribute('aria-expanded', 'true');
  const source = math.locator('.cherry-math__source code');
  await expect(source).toBeVisible();
  await source.fill('z=3');
  await expect.poll(() => markdown(page)).toContain('z=3');
  await expect(block).toContainText('z=3');
  await sourceToggle.click();
...
for (const item of sourceCases) {
  test(`source-backed ${item.name} supports focus, update, close and deletion`, async ({ page }) => {
    ...
    const toggle = root.getByRole('button', {
      name: item.root.includes('cherry_native_block') ? '编辑整个区块源码' : '在节点内编辑源码',
      exact: true,
    });
    await toggle.click();
