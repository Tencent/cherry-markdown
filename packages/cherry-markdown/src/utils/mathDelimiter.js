...
/**
 * @param {string} str
 * @param {Record<string, { replacement: string, selfClosing?: boolean }>} replacements
 */
export function normalizeMathDelimiters(str, replacements) {
  let delimiters = delimiterPairs.map((delimiter) => ({
    ...delimiter,
    ...replacements[delimiter.open],
  }));
  let result = '';
  let cursor = 0;
  ...
  while (cursor < str.length) {
    ...
    if (closeIndex === -1) {
      ...
      const hasContent = content.replace(/CHERRYFLOWSESSIONCURSOR\n*$/, '').trim();
      if (nextDelimiter.replacement && nextDelimiter.selfClosing && hasContent && !isInLinkDestination) {
        return result + nextDelimiter.replacement + content;
      }
