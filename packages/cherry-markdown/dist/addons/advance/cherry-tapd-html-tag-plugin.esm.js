...
//#endregion
//#region src/addons/advance/cherry-tapd-html-tag-plugin.js
var v = class extends _ {
	constructor() {
		super({ needCache: !0 });
	}
	makeHtml(e, t) {
		return e.replace(this.RULE.reg, (e) => {
			let n = e.split(/[\r\n]/).length, r = e.replace("[html]", "").replace("[/html]", "").replace(/[\r\n]/g, "").replace(/&#60;/g, "<").replace(/&#62;/g, ">").replace("style=\"LAYOUT-GRID", "style=\"");
			return r = this._trimScripTag(r), `<div data-lines="${n}" data-sign="${t(r).sign}">${r}</div>`;
		});
	}
	rule() {
		return { reg: /\[html\]([\s\S]*?)\[\/html\]/g };
	}
	_trimScripTag(e) {
		return e.replace(/(<(?:script|script\s[^>]*)>)([\s\S]*?)(<\/script>)/gi, "").replace(/(<iframe [^>]*>)|(<\/iframe>)/gi, "");
	}
};
r(v, "HOOK_NAME", "tapdHtmlTag");
//#endregion
export { v as default };
