 ...
 * 3. 拦截对 dist/cherry-markdown.stream.js 的请求，重定向到虚拟模块（源码入口 index.stream.js）
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use((req, res, next) => {
        ...
        // 4. 拦截 cherry-markdown.js（非 core/stream）请求 → full 虚拟模块
...
`;
    ...
    transformIndexHtml(html) {
      ...
      // 2. cherry-markdown.js / core.js / stream.js（开发模式映射为源码模块）→ type="module"
      result = result.replace(
        /<script([^>]*)\s+src=["']([^"']*\/packages\/cherry-markdown\/dist\/cherry-markdown(?:\.core|\.stream)?\.js)["']([^>]*)><\/script>/gi,
