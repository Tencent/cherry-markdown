...
| Cherry 胶水层 | `cherry-markdown` → `addons/cherry-searcher-plugin.js` | `usePlugin` 注册、`searcherBridge`、CodeMirror 适配 |

未调用 `Cherry.usePlugin(SearcherCherryPlugin)` 时，工具栏按钮可见但点击无效。未在 `toolbar` 中配置 `'searcher'` 时，按钮与 Mod+F / Mod+H 快捷键均不可用；完全自定义 toolbar 时需自行加入该项。
...
在 `new Cherry()` **之前**注册插件；样式需单独引入 addon 构建产物：
...
const cherry = new Cherry({
  id: 'editor',
  toolbars: {
    // 默认配置已含 'searcher'；完全自定义 toolbar 时需自行加入
    toolbar: ['bold', 'italic', '|', 'searcher', 'togglePreview'],
  },
});
```

- **工具栏按钮**：点击搜索图标，以按钮位置为锚点弹出面板
- **Mod+F**：打开 / 关闭搜索面板
...
通过 `Cherry.usePlugin(SearcherCherryPlugin, options)` 传入，并写入 `toolbars.config.searcher`：
